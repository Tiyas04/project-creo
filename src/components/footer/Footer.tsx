...
const Footer = () => {
  return (
    <footer className='relative w-full  overflow-hidden rounded-t-[28px]'>
      {/* Background Image (Car is embedded in this image) */}
      <img
        src={imgs.bgImage}
        alt='Footer background'
        className='absolute inset-0 w-full h-full sm:object-center'
        width='100%'
        height='100%'
      />
      ...
      {/* CONTENT */}
      <div className='relative z-10 px-4 py-6 md:px-12 md:py-12 text-white overflow-hidden'>
        {/* Main Content Area */}
        <div className='flex flex-col md:flex-row justify-between items-start gap-8 relative font-inter'>
          {/* Left Side: Logo & Socials */}
          <div className='flex flex-col gap-6 max-w-lg z-20'>
            {/* Logo Section */}
            <div>
              <img
                src={imgs.creotextHolder}
                alt="CREO '26 Design Conference"
                className='h-auto w-48 sm:w-64 md:w-80 object-contain'
              />
            </div>

            {/* Social Links */}
            <div className='flex flex-wrap items-center gap-4 md:gap-6 text-xs sm:text-sm md:text-base font-bold tracking-wide text-white font-inter'>
              {/* Instagram */}
              <a
            ...
            <div className='flex flex-col gap-6 mt-8'>
              {/* Phone Numbers */}
              <div className='flex flex-col md:flex-row gap-4 md:gap-16 text-[10px] sm:text-xs md:text-sm font-bold tracking-wider'>
                {/* Person 1 */}
                <div className='flex items-center gap-4'>
                  <div className='bg-white/10 p-2.5 rounded-xl backdrop-blur-md border border-white/10'>
                    <img src={imgs.phone} alt='Phone' className='w-10 h-10' />
                  </div>
                  <div className='flex flex-col gap-1'>
                    <Small className='text-white/60 text-[10px] font-extrabold uppercase tracking-widest font-inter'>
                      SOMANATH MAHANTA
                    </Small>
                    <a
              ...
              <div className='mt-2 text-left'>
                <img src={imgs.contacttext} alt='Contact Us' className='h-5 object-contain' />
              </div>
            </div>
          </div>
        </div>

        {/* Credits (Absolute Bottom Right) */}
        <div className='absolute bottom-6 right-8 md:right-12 z-20 opacity-80'>
          <img
            src={imgs.textdsc}
